...
- **Functions**. Easy, simple, clean functions are defined with def. But, if you want to pass around functions, you have to study the concept of lambda , which is a keyword that allows you to create one-line(-only?) functions. Other things that are worth noting:
  - to document functions, python devs have thought of _docstrings_, which I find a terrible idea, but that's it.
...
- **Data structures**
  - The most used is the _list_ for sure, which behaves like an array in JS, but there are also dictionaries (javascript objects), and sets, and many more, you name it, you get it.
